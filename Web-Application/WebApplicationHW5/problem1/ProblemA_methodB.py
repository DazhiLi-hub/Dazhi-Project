...
def fit(X, Y, w):
    ...
    #w_grad return gradient, j return cost, h_total return predict_result

def error_cal(y_true,y_pre):
    return (np.power(y_true-y_pre.reshape(1,4),2)).sum()/4
...
def main():
    ...
    while(error_rate>e):
        ...
        error_rate=error_cal(Y,h_current)
    ...
    print("\nThe first batch error is: "+str(errors[0]))
    print("The final batch error is: "+str(errors[-1]))
    print("\nThe number of batches are: "+str(len(result['J']))+"\n")
